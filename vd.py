#!/usr/bin/env python3
"""
vd.py (OAK-D + MiDaS Mono Depth fallback version)
...
def tts_worker(tts_queue):
    try:
        ...
        # Optional: adjust rate if desired by modifying speaker.Rate
        while True:
            text = tts_queue.get()
            if text is None:
                break
...
def depth_to_grayscale_midas(disparity: np.ndarray, params: DepthParams, state: dict) -> np.ndarray:
    ...
    k = int(params.median_ksize)
    if k >= 3 and (k % 2 == 1):
        d = cv2.medianBlur(d, k)
...
def depth_to_grayscale_oak(depth_mm: np.ndarray, params: DepthParams, state: dict) -> np.ndarray:
    ...
    k = int(params.median_ksize)
    if k >= 3 and (k % 2 == 1):
        depth_mm = cv2.medianBlur(depth_mm, k)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="OAK-D / MiDaS Depth -> vOICe-matched window")
    ...
    ap.add_argument("--auto-smooth", type=float, default=0.85)
...
def main() -> int:
    ...
    params = DepthParams(
        ...
        auto_smooth=float(args.auto_smooth),
    ...
    cv2.resizeWindow(args.window, int(args.window_w), int(args.window_h))
    try:
        cv2.setWindowProperty(args.window, cv2.WND_PROP_TOPMOST, 1)
    except Exception:
        pass
    ...
    gamma_cycle = [0.50, 0.60, 0.75, 1.00]
        ...
        while True:
            ...
            if gray is None:
                if last_gray is not None:
                    ...
                    gray = np.zeros((args.height, args.width), dtype=np.uint8)
                    cv2.putText(gray, "Waiting for camera...", (20, args.height // 2), 
            ...
            if k in (ord("q"), 27):
                ...
                gamma_idx = (gamma_idx + 1) % len(gamma_cycle)
                params.gamma = gamma_cycle[gamma_idx]
                print(f"Gamma set to: {params.gamma}")
                tts_queue.put(f"Gamma {params.gamma}")
            elif k == ord("v"):
                ...
                if use_oak:
                    params.manual_near_oak = max(1, params.manual_near_oak - 50)
                    tts_queue.put(f"Near limit decreased to {params.manual_near_oak}")
                else:
                    params.manual_near_midas *= 0.9
                    tts_queue.put(f"Near limit decreased to {int(params.manual_near_midas)}")
            elif k == ord("]"):
                params.auto_range = False
                if use_oak:
                    params.manual_near_oak = min(params.manual_far_oak - 1, params.manual_near_oak + 50)
                    tts_queue.put(f"Near limit increased to {params.manual_near_oak}")
                else:
                    params.manual_near_midas *= 1.1
                    tts_queue.put(f"Near limit increased to {int(params.manual_near_midas)}")
            elif k == ord("-"):
                params.auto_range = False
                if use_oak:
                    params.manual_far_oak = max(params.manual_near_oak + 1, params.manual_far_oak - 250)
                    tts_queue.put(f"Far limit decreased to {params.manual_far_oak}")
                else:
                    params.manual_far_midas *= 0.9
                    tts_queue.put(f"Far limit decreased to {int(params.manual_far_midas)}")
            elif k == ord("="):
                params.auto_range = False
                if use_oak:
                    params.manual_far_oak = min(20000, params.manual_far_oak + 250)
                    tts_queue.put(f"Far limit increased to {params.manual_far_oak}")
                else:
                    params.manual_far_midas *= 1.1
                    tts_queue.put(f"Far limit increased to {int(params.manual_far_midas)}")
            elif k == ord("r"):
                params.invert = True
                ...
                if last_gray is not None:
                    out = f"voice_depth_capture_{int(time.time())}.png"
                    cv2.imwrite(out, last_gray)
                    print(f"Saved: {out}")
                    tts_queue.put(f"Saved frame")
